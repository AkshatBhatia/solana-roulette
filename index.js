            ...
            when:async (val)=>{
                if(parseFloat(totalAmtToBePaid(val.SOL))>5){
                    ...
                    if(userBalance<totalAmtToBePaid(val.SOL)){
                        ...
                        console.log(chalk.green`Your balance is ${userBalance}`);
...
const gameExecution=async ()=>{
    ...
    if(answers.RANDOM){
        ...
        if(answers.RANDOM===generateRandomNumber){
            ...
            const userBalance=await getWalletBalance(userWallet.publicKey.toString())
            console.log(chalk.green`Your new balance is ${userBalance}`);
        }else{
            //better luck next time
            console.log(chalk.yellowBright`Number was ${generateRandomNumber}`);
